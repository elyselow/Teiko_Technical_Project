...
Note 2: If make dashboard is run before make pipeline, cell_counts.db will already be created in the repository environment since it is required in the dashboard code to make the dashboard URL. Thus, cell_counts.db must be removed from the Codespaces environment before make pipeline is run to prevent "table already exists" error.
